...
const KNOWN_PLACES = [
  ...
  { lat: 15.4, lon: 73.88, r: 25, city: 'Goa', region: 'Goa', country: 'India', emoji: '🇮🇳' },
  {
    lat: 13.08,
    lon: 80.27,
    r: 25,
  ...
  { lat: 39.1, lon: -75.44, r: 15, city: 'Dover', region: 'Delaware', country: 'USA', emoji: '🇺🇸' },
  {
    lat: 39.63,
    lon: -78.23,
    r: 20,
  ...
  // ─── UAE ───
  { lat: 25.2, lon: 55.27, r: 25, city: 'Dubai', region: 'Dubai', country: 'UAE', emoji: '🇦🇪' },
  {
    lat: 24.99,
    lon: 55.72,
    r: 25,
...
function main() {
  ...
  const rawStops = clusters
    ...
    .map((c, i) => {
      ...
      let name = geo
        ? geo.city
        : `${Math.abs(c.lat).toFixed(2)}°${c.lat >= 0 ? 'N' : 'S'}, ${Math.abs(c.lon).toFixed(2)}°${c.lon >= 0 ? 'E' : 'W'}`;
      ...
      const cityInfo = cityDb[name] || {
        tagline: 'Travel Destination',
        highlight: 'Explored this beautiful location',
        icon: '📍',
      };
      return {
        id: i + 1,
  ...
  for (let i = 1; i < stops.length; i++) {
    ...
    const curr = stops[i];
    curr.distFromPrev = Math.round(
      haversine(prev.coordinates[1], prev.coordinates[0], curr.coordinates[1], curr.coordinates[0])
    );
  ...
  for (const s of stops) {
    if (!countryMap[s.country])
      countryMap[s.country] = { emoji: s.emoji, cities: new Set(), photos: 0 };
  ...
  console.log(
    `⚡ ${stops.length} stops → ${uniqueCities} unique cities → ${countries.length} countries`
  );
