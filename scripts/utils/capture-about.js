...
async function run() {
  ...
  const defaultTheme = await dPage.evaluate(() => document.documentElement.classList.contains('dark') ? 'dark' : 'light');
  ...
  const mDefaultTheme = await mPage.evaluate(() => document.documentElement.classList.contains('dark') ? 'dark' : 'light');
