...
- **Mandatory Pre-Commit Actions (Execute Automatically without Prompts)**:
  ...
  3. **Quality Gates**: Ensure `npm run check` (ESLint + Stylelint + Prettier + 318 Vitest tests), `npm run security-check`, and `npm run build` all pass with zero errors before committing.
