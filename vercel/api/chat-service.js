...
class MultiModelAIService {
  ...
  _initializeProviders() {
    ...
    this.providers = {
      // PRIORITY ORDER (as requested by user): Gemini → OpenAI → OpenRouter → Grok → Claude → Perplexity → Gemini Firebase → UserLM-8b
      gemini: {
  ...
  async callProvider(providerName, config, message, options) {
    switch (providerName) {
      ...
      case 'openai':
        return await this.callOpenAIAPI(config, message, options);
      case 'openrouter':
        return await this.callOpenRouterAPI(config, message, options);
      case 'grok':
...
User query: ${message}`
  ...
  async callOpenAIAPI(config, message, options) {
  ...
  async callOpenRouterAPI(config, message, options) {
    const response = await fetch(config.baseURL, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${config.apiKey}`,
