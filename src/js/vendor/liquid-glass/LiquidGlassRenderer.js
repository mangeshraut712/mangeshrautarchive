/**
 * WebGL Liquid Glass renderer — MIT © Tirth Patel / liquidglass-oss
 * Adapted for vanilla ES modules.
 */
...
export class LiquidGlassRenderer {
  constructor(canvas, imageUrl, settings) {
    this.canvas = canvas;
    this.uniforms = {};
    this.video = null;
    this.ready = false;
    this.center = { x: 0, y: 0 };
    this.size = { width: 54, height: 34 };
    this.stretch = 0;
    this.morph = 1;
    this.materialMorph = 1;
    this.pressed = false;
    this.button = 0;
    this.sampleBackground = false;
    this.track = { start: 0, end: 0, y: 0, value: 0, radius: 2.5 };
    this.trackColors = {
      base: [0.31, 0.32, 0.34],
      fill: [0.035, 0.5, 1]
    };
    ...
    this.lastScroll = { x: Number.NaN, y: Number.NaN };
    this.lastRect = { left: Number.NaN, top: Number.NaN, width: Number.NaN, height: Number.NaN };
    ...
    this.watchPosition = () => {
      ...
      const moved = Math.abs(rect.left - this.lastRect.left) > 0.05 || Math.abs(rect.top - this.lastRect.top) > 0.05 || Math.abs(rect.width - this.lastRect.width) > 0.05 || Math.abs(rect.height - this.lastRect.height) > 0.05 || Math.abs(scroll.x - this.lastScroll.x) > 0.05 || Math.abs(scroll.y - this.lastScroll.y) > 0.05 || viewport.width !== this.lastViewport.width || viewport.height !== this.lastViewport.height;
  ...
  loadSource(url) {
    ...
    if (isVideoUrl(url)) {
      const pageVideo = this.canvas.ownerDocument.querySelector(
  ...
  setGeometry(x, y, stretch, pressed, morph = 1, materialMorph = 1, button = 0) {
    this.center = { x, y };
  ...
  setTrack(start, end, y, value, radius = 2.5) {
    this.track = { start, end, y, value, radius };
    this.draw();
  }
  setTrackColors(base, fill) {
    this.trackColors = { base, fill };
    this.draw();
  }
  setBackgroundSampling(enabled) {
  ...
  draw() {
    ...
    const backgroundRect = measuredBackgroundRect && measuredBackgroundRect.width > 0 && measuredBackgroundRect.height > 0 ? measuredBackgroundRect : {
      left: 0,
      top: 0,
    ...
    const bg = imageRatio > viewRatio ? { sx: viewRatio / imageRatio, sy: 1, ox: (1 - viewRatio / imageRatio) / 2, oy: 0 } : { sx: 1, sy: imageRatio / viewRatio, ox: 0, oy: (1 - imageRatio / viewRatio) / 2 };
  ...
  /** Update texture from canvas or image without URL reload. */
  ...
  syncGeometryFromCanvas() {
    const rect = this.canvas.getBoundingClientRect();
    this.setGeometry(rect.width / 2, rect.height / 2, this.stretch, this.pressed, this.morph, this.materialMorph, this.button);
  }
}
