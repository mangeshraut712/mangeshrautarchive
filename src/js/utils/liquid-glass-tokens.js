...
export function syncLiquidGlassTokens(tintRatio, { instant = false } = {}) {
  ...
  // Clear = heavy blur + low fill (see-through). Tinted = light blur + high fill (frosted).
  ...
  // Card alpha: 12% clear → 98% tinted (Apple clear vs regular material)
  const cardLightAlpha = Math.round(12 + t * 86);
  const cardDarkAlpha = Math.round(16 + t * 82);
  style.setProperty('--lg-card-light-alpha', `${cardLightAlpha}%`);
  style.setProperty('--lg-card-dark-alpha', `${cardDarkAlpha}%`);
