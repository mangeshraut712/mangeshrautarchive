...
class ChatUI {
  constructor() {
    ...
    if (this.elements.input) {
      this.modernInput = new ModernInputHandler(this.elements.input, () => this._handleUserInput());
    }
  ...
  _initializeElements() {
    ...
    const form =
      document.getElementById('chat-form') || document.getElementById('portfolio-chat-form');
    const input =
  ...
  _bindEvents() {
    ...
    document.addEventListener('click', event => {
      ...
      if (
        event.target.closest('#message-input') ||
    ...
    if (this.elements.voiceButton) {
      ...
      voiceButton.addEventListener('pointerdown', event =>
        this._handleVoiceButtonPointerDown(event)
      );
    ...
    if (this.elements.voiceOutputToggle) {
      this.elements.voiceOutputToggle.addEventListener('change', event => {
        ...
        document.dispatchEvent(
          new CustomEvent('voice-output-toggle', {
            detail: { enabled },
          })
        );
      });
    ...
    if (this.elements.voiceContinuousToggle) {
      this.elements.voiceContinuousToggle.addEventListener('change', event => {
        const enabled = Boolean(event.target.checked);
        document.dispatchEvent(
          new CustomEvent('voice-continuous-toggle', {
            detail: { enabled },
          })
        );
      });
    ...
    if (muteBtn) {
      muteBtn.addEventListener('click', () => {
        if (window.voiceManager) {
          ...
          this.addMessage('🔇 Voice output muted', 'system', {
            skipSpeech: true,
          });
          this.hideVoiceMenu();
        }
    ...
    if (stopBtn) {
      stopBtn.addEventListener('click', () => {
        if (window.voiceManager) {
          ...
          window.voiceManager.stopSpeaking();
          this.addMessage('⏹️ Voice mode stopped', 'system', {
            skipSpeech: true,
          });
          this.hideVoiceMenu();
        }
    ...
    if (this.elements.widget) {
      ...
      document.addEventListener('click', event => {
        ...
        if (
          event.target.closest('#portfolio-chat-widget') ||
  ...
  _createMessageElement(content, role, timestamp, metadata = {}, isStreaming = false) {
    ...
    if (Array.isArray(metadata.providers) && metadata.providers.length) {
      ...
      const isRedundant =
        metadata.source && providerText.toLowerCase().includes(metadata.source.toLowerCase());
  ...
  _formatAssistantResponse(response) {
    ...
    metadata.model = response.model || 'x-ai/grok-4.1-fast';
  ...
  _normalizeAnswerPayload(response) {
    const value = response?.html
      ? { html: response.html }
      : (response?.answer ?? response?.text ?? response);
