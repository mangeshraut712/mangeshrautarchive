class SkillsVisualization {
  ...
  createSkillBadge(e, l) {
    const a = e.level;
    return `\n      <div class="skill-badge group" style="animation-delay: ${50 * l}ms">\n        <div class="skill-badge-icon" style="color: ${e.color}; background: ${e.color}15; border-color: ${e.color}30;">\n          <i class="${e.icon}"></i>\n        </div>\n        <div class="skill-badge-content">\n          <div class="skill-badge-name">${e.name}</div>\n          <div class="skill-badge-level">\n            <div class="skill-badge-bar">\n              <div class="skill-badge-progress" style="transform: scaleX(${a / 100}); background: ${e.color}"></div>\n            </div>\n            <span class="skill-badge-percentage">${a}%</span>\n          </div>\n        </div>\n      </div>\n    `;
  }
  createCategorySection(e, l, a) {
    return `\n      <div class="skill-category mb-8">\n        <h3 class="text-xl font-bold mb-4 text-primary pb-2">\n          ${e}\n        </h3>\n        <div class="skill-scroll-container">\n          <div class="skill-scroll-wrapper">\n            ${[...l, ...l, ...l].map((e, l) => this.createSkillBadge(e, l)).join('')}\n          </div>\n        </div>\n      </div>\n    `;
  }
  render(e = 'skills-container') {
    const l = document.getElementById(e);
    if (!l) return void console.error(`Container with id "${e}" not found`);
    const a = Object.entries(this.skills)
      .map(([e, l], a) => this.createCategorySection(e, l, a))
      .join('');
    ((l.innerHTML = a), this.injectStyles());
  }
  injectStyles() {}
  getStats() {
    const e = Object.values(this.skills).flat(),
      l = e.length;
    return {
      totalSkills: l,
      avgProficiency: Math.round(e.reduce((e, l) => e + l.level, 0) / l),
      expertSkills: e.filter(e => e.level >= 85).length,
      categories: Object.keys(this.skills).length,
    };
  }
}
('undefined' != typeof module && module.exports && (module.exports = SkillsVisualization),
