...
class BlogLoader {
    ...
    renderPosts() {
        ...
        this.container.innerHTML = postsToShow.map(post => `
                ...
                <div class="blog-card-content">
                    ...
                    <div class="apple-tldr">
                        <div class="tldr-header">
                            ...
                            <span style="font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: var(--apple-accent);">AI Summary</span>
                        </div>
                    ...
                    <div class="blog-tags" style="margin-top: 1rem; margin-bottom: 1.5rem; display: flex; flex-wrap: wrap; gap: 0.5rem;">
                        ${post.tags.map(tag => `<span class="blog-tag" style="font-size: 0.7rem; padding: 0.2rem 0.6rem; border-radius: 980px; border: 1px solid var(--apple-border-light); background: var(--apple-bg-light); color: var(--text-color-secondary);">${tag}</span>`).join("")}
                    </div>
        ...
        let viewMoreBtn = document.getElementById("blog-view-more");
        if (this.allPosts.length > this.limit) {
            if (!viewMoreBtn) {
                viewMoreBtn = document.createElement("button");
                viewMoreBtn.id = "blog-view-more";
                viewMoreBtn.className = "btn hero-cta";
                viewMoreBtn.style.margin = "3rem auto 0";
                viewMoreBtn.style.display = "flex";
                viewMoreBtn.onclick = () => this.toggleExpand();
                this.container.after(viewMoreBtn);
            }
            viewMoreBtn.innerHTML = this.isExpanded ? 
                `Show Less <i class="fas fa-chevron-up" style="margin-left: 8px;"></i>` : 
                `View More Blogs (${this.allPosts.length - this.limit} more) <i class="fas fa-chevron-down" style="margin-left: 8px;"></i>`;
        }
    }
        ...
        body.innerHTML = `
            <header class="article-header" style="margin-bottom: 2.5rem; text-align: left;">
                ...
                <div class="article-tags" style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                    ${post.tags.map(tag => `<span class="blog-tag" style="font-size: 0.75rem; padding: 0.3rem 0.8rem; border-radius: 980px; border: 1px solid var(--apple-border-light); background: var(--apple-bg-light);">${tag}</span>`).join("")}
                </div>
            </header>
    ...
    parseContent(content) {
        return content
            ...
            .replace(/`([^`]+)`/gim, '<code style="background: var(--apple-border-light); padding: 0.2rem 0.4rem; border-radius: 4px; font-family: monospace;">$1</code>')
            .replace(/```([\s\S]*?)```/gm, '<pre style="background: var(--apple-bg-dark); color: #fff; padding: 1.5rem; border-radius: 12px; overflow-x: auto; margin: 1.5rem 0;"><code>$1</code></pre>')
            .replace(/\n/gim, '<br>');
    }
